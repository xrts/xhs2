function signUpGroup(container) {
	...
	this.success = null;
	this.getPicVerifyUrl = 'http://wx.house.ifeng.com/sale/api/getverifycode?s=';
	this.getValidverifycode = 'http://wx.house.ifeng.com/sale/api/validverifycode';
	this.openId = null; //活动ID
}
	...
	bind: function() {
		...
		this.submitBtn.on('click', function(){
			...
			if (me.check()) {
				...
				$.ajax({
					type: "GET",
					url: "http://wx.house.ifeng.com/signup/api/apply",
					dataType: 'jsonp',
					jsonp: "_cb",
					data: {
						id: me.activityId,
						packid: me.activityId,
						openid: me.openId,
						post:me.ajaxData
					},
		...
		this.getCodeBtn.on("click", function() {	
			...
			if(mobileFlag == ''){
				...
				if(me.isPC()){
					...
					$.ajax({
						type: "POST",
						url: "http://wx.house.ifeng.com/sale/api/validverifycode",
						dataType: 'jsonp',
						jsonp: "_cb",
						data: {
							"code": imgmessage,
						...
						success: function(json) {
							if (json.errno == 0) {
								...
								$.ajax({
									type: "POST",
									url: "http://wx.house.ifeng.com/signup/api/sendidentifyingcode",
									dataType: 'jsonp',
									jsonp: "_cb",
									data: {
										"signUpMobile": mobile,
	...
	addDetail: function() { //添加报名团内容 姓名 手机号 验证码
		...
		if(this.isPC()){
			this.html += '<li>' +
							...
							'<img id="getimgCode" src="http://wx.house.ifeng.com/sale/api/getverifycode?s=0.4980527254723368" alt="">' +
						'</li>';
		}
